...
[a test proves the wrong scale is caught](tests/test_live_fixture.cpp) rather
...
| | p50 | p99 | p99.9 | max |
|---|---|---|---|---|
| Median of 5 runs, `--realtime` | 2.4 us | 18.7 us | 95.6 us | 125 us |
...
The tool still warns that it fell behind on a handful of events, and that
warning is generated by the harness rather than added afterwards. **Do not
compare these to a production system**: turbo and C-states are untouched, the
capture is read from disk, and the run-to-run spread on the tail is roughly 5x.
The median is stable; the tail is measuring this desktop.
...
- [x] 235 test cases / 92k assertions, `-Werror`, ASan + UBSan, six fuzz targets
