...
TEST_CASE("Replaying the capture is bit-identical across platforms", "[fixture][determinism]") {
    ...
    // FNV-1a over the side tag, level count, a domain separator, and then the
    // mantissas of every level in book order. No floating point is involved
    // anywhere upstream of this, so a difference here is a defect and never a
    // rounding difference.
    //
    // This constant changed once, deliberately: it was 0x080281c2dd87183f
    // before `state_hash` began mixing the level count and separator. Without
    // them the side tag was indistinguishable from a mantissa, and one bid at
    // (1,1) hashed identically to one ask at (1,1) — see
    // tests/test_determinism.cpp. That is a hash-format break, not a book
    // change: the same capture produces the same book, and only the
    // fingerprint of it moved. Any other change to this number is a bug.
    CHECK(feed.book().state_hash() == 0xE9613AF632F40653ULL);
}
