...
std::string diff_frame(std::int64_t event_us, std::int64_t first, std::int64_t last,
                       std::int8_t price_exp, std::int8_t qty_exp,
                       const std::vector<Level>& bids, const std::vector<Level>& asks,
...
BinanceSbeDecoder btcusdt() { return BinanceSbeDecoder(InstrumentSpec{"BTCUSDT", 2, 8}); }
...
TEST_CASE("a coarser exponent rescales exactly", "[venues][binance][sbe]") {
    ...
    const std::string frame =
        diff_frame(1, 100, 105, -1, -8, {{452'835, 50'000'000}}, {});
    const DecodedMessage& msg = decoder.decode(frame);
    REQUIRE(msg.ok());
...
TEST_CASE("a foreign symbol is ignored before any level is trusted",
          "[venues][binance][sbe]") {
    auto decoder = btcusdt();
    const std::string frame =
...
TEST_CASE("other templates on the socket are routing, not failure",
          "[venues][binance][sbe]") {
    auto decoder = btcusdt();
    Wire w;
...
TEST_CASE("truncation anywhere is malformed, never a partial decode",
          "[venues][binance][sbe]") {
    auto decoder = btcusdt();
    const std::string whole = diff_frame(1, 100, 105, -2, -8, {{4'528'350, 50'000'000}},
                                         {{4'528'360, 30'000'000}});
...
TEST_CASE("the SBE decoder drives a feed end to end", "[venues][binance][sbe][feed]") {
    ...
    Feed<BinanceSbeDecoder, ArrayBook> feed("binance-sbe", btcusdt(),
                                            SequencePolicy::kBinanceSpot);
